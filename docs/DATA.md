...
| `testcenter.json`               | Mock-exam test-centre index: paper catalogue, Cambridge holdings matrix, hand-tagged question taxonomy, exam-shell timing budgets and the platform's production score calibration.                                                                                                                    |
| `wordbook.json`                 | Community wordbook index: 4,323 Cambridge 1-18 headwords as shipped by `Iamdacai/ielts-vocab-system`, with completeness counts, a per-volume cross-validation against `vocabulary.json`, and the upstream data-quality audit. Headwords and statistics only; no upstream definition is redistributed. |
