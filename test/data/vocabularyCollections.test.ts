...
describe('collection statistics consistency', () => {
  it('computes mean size from all memberships, including overlapping volume collections', () => {
    const stats = vocabularyCollectionsStats();
    const memberships = Object.values(stats.wordsPerCollection).reduce((sum, count) => sum + count, 0);
    expect(memberships).toBeGreaterThan(stats.totalHeadwords);
    expect(stats.meanWordsPerCollection).toBe(Math.round((memberships / stats.totalCollections) * 100) / 100);
  });

  it('supports missing glosses and morphemes without an ignored coverage branch', () => {
    const entry = { ...allEntries()[0]!, definition: null, morphemes: null };
    expect(sceneForWord(entry)).toBe(sceneForWord({ ...entry, definition: '', morphemes: '' }));
  });
});
