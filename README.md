...
grey-literature archive index, and [`wanli4473/yysd-testcenter`](https://github.com/wanli4473/yysd-testcenter)
for the operating-platform index. None is redistributed: the API publishes derived, non-substitutive
...
24 marked learner essays summarised statistically. And it indexes an operating product:
`/v1/platform` exposes the 377-page content manifest, vocabulary-theme catalogue, speaking recall
bank and A-Level past-paper catalogue of a live IELTS mock-exam platform, and `/v1/listening`
makes its 530-section listening taxonomy — seven question types, sixteen scenes, three
difficulties — queryable without redistributing a single exam page or audio file.
...
# An operating mock-exam platform: 377 pages by zone and subject, 36 vocab themes, 95 speaking topics
curl -s "http://localhost:3000/v1/platform/manifest/items?zone=mock&limit=2"
curl -s "http://localhost:3000/v1/platform/vocab-themes/items?category=exam&limit=2"

# A bilingual listening taxonomy: seven types, sixteen scenes, three difficulties, 530 sections
curl -s "http://localhost:3000/v1/listening/types"
curl -s "http://localhost:3000/v1/listening/groups?volume=5&part=1"
```
...
| Operating platform index        | 377 pages / 36 vocab themes / 95 speaking topics / 852 A-Level papers / 2 Ebbinghaus schedules | `/v1/platform`          | Derived metadata index of the live mock-exam platform [wanli4473/yysd-testcenter](https://github.com/wanli4473/yysd-testcenter) |
| Listening taxonomy              |                          530 sections / 2,720 questions / 7 types / 16 scenes / 4 difficulties | `/v1/listening`         | Bilingual listening annotation (volume / test / part / question range / type / scene / difficulty)                              |
...
| GET    | `/v1/platform`                    | Operating-platform provenance, aggregate statistics and facets                                          |
| GET    | `/v1/platform/stats`              | Operating-platform statistics only                                                                      |
| GET    | `/v1/platform/manifest`           | Manifest statistics and facets                                                                          |
| GET    | `/v1/platform/manifest/items`     | Search the 377-page manifest (`zone`, `subject`, `q`, `sort`)                                           |
| GET    | `/v1/platform/manifest/:id`       | One manifest page by identifier                                                                         |
| GET    | `/v1/platform/vocab-themes`       | Vocabulary-theme catalogue categories and statistics                                                    |
| GET    | `/v1/platform/vocab-themes/items` | Search the 36 vocabulary themes (`category`, `q`, `sort`)                                               |
| GET    | `/v1/platform/vocab-themes/:id`   | One vocabulary theme by identifier                                                                      |
| GET    | `/v1/platform/speaking`           | Quarterly speaking recall bank overview and statistics                                                  |
| GET    | `/v1/platform/alevel`             | A-Level past-paper catalogue by board                                                                   |
| GET    | `/v1/platform/alevel/:id`         | One A-Level board by identifier                                                                         |
| GET    | `/v1/platform/schedules`          | Ebbinghaus spaced-repetition schedules                                                                  |
| GET    | `/v1/listening`                   | Listening taxonomy overview: statistics and facets                                                      |
| GET    | `/v1/listening/stats`             | Listening taxonomy statistics only                                                                      |
| GET    | `/v1/listening/types`             | Seven listening question types (bilingual) with observed frequencies                                    |
| GET    | `/v1/listening/types/:id`         | One listening question type by English identifier                                                       |
| GET    | `/v1/listening/scenes`            | Sixteen listening scenes / topical contexts with observed frequencies                                   |
| GET    | `/v1/listening/scenes/:id`        | One listening scene by English identifier                                                               |
| GET    | `/v1/listening/diffs`             | Difficulty tiers for listening sections                                                                 |
| GET    | `/v1/listening/groups`            | Search the 530 listening section groups (`volume`, `part`, `qType`, `scene`, `diff`, `q`)               |
| GET    | `/v1/listening/groups/:id`        | One listening section group by identifier                                                               |
...
CI re-derives the vocabulary, study-materials, archive and operating-platform datasets from upstream on every push — the
archive derivation downloads the 38 document blobs it needs by blob SHA and the platform derivation downloads its 7 committed blobs — and fails if any committed
...
- **100% coverage** — statements, branches, functions and lines, enforced per file by the test
  runner (`npm test` fails below 100%). 577 tests, zero runtime dependencies.
...
- **Upstream files:** never redistributed. `/v1/corpus`, `/v1/tests`, `/v1/archive`, `/v1/platform` and `/v1/listening` publish
  derived metadata and statistics only — no passage, question, answer key, transcript, recording,
  PDF content, essay text, HTML exam page, audio file, dictionary gloss or college essay.
