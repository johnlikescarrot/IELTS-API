...
## Part VIII — the vocabulary-learning layer

### 42. What an operational vocabulary-learning system contains

Parts I-VII indexed what other people _publish_; the design study behind this part is a system that
_practises_: [`Iamdacai/ielts-vocab-system`](https://github.com/Iamdacai/ielts-vocab-system) (雅思智能
背单词系统), a production WeChat mini-program with a Node.js/SQLite backend, examined at HEAD
`1f5ad56` on 2026-09-07. Its README documents the pedagogy: learners pick a word bank, study a
fixed number of words a day, and are re-tested on an Ebbinghaus review schedule. The working
database (`backend/ielts_vocab.db`, committed alongside the source) holds 47,044 word rows for
school-exam and IELTS lists identified by category labels such as `GRE 单词表`; the Cambridge
per-volume wordbook ships separately as `backend/scripts/ielts-4000-vocabulary.json` (4,323
entries carrying a `cambridge_book` field), and the 雅思词汇真经 bank — 3,674 words in 22 scenario
themes, integrated on 2026-03-14 from `hefengxian/my-ielts` with 480 MB of chapter and word audio —
is documented with a per-row field schema (`category` like `01_自然地理`, `meaning`, `example`).
Around those banks sit a 3,099-row verb-noun collocation table, 167 example sentences, speaking
and writing prompt banks, and per-user progress, review-session and pronunciation records.

Three findings shaped what this API would and would not take from that system. First, the
_method_ is sound and worth publishing as reproducible mathematics: daily quotas plus an expanding
review ladder is the standard spaced-practice design (see §44). Second, the _content_ is not
reusable: the repository declares no licence, the 真经 bank restates a commercial vocabulary book,
and the committed database contains live learner rows — copying any of it would poison this
project's CC BY data licence and its privacy posture. Third, the machine-generated practice rows
show what unvalidated content does to quality: collocation entries such as "You can Abandon
attempt." and example sentences such as "This is an example sentence for 1" are exactly the kind of
noise a research API must not publish. The layer below therefore re-implements the method as pure,
stateless code over this API's own CC BY 4.0 Cambridge headword set, and publishes nothing but the
method's outputs.

### 43. The recurrence structure of the Cambridge headword list

Before scheduling words, the layer needed a frequency-like signal. The corpus's only
within-corpus signal is recurrence: how many Cambridge volumes a headword appears in
(`volumes` in every entry of `data/vocabulary.json`). Measuring it is a result in itself:

| Volumes containing the headword | Headwords | Share of 4,174 |
| ------------------------------: | --------: | -------------: |
|                               1 |     4,041 |          96.8% |
|                               2 |       132 |           3.2% |
|                               3 |         1 |         < 0.1% |
...
The per-volume lists of Cambridge IELTS 1-22 are almost disjoint: 4,174 headwords yield only 4,310
word-volume occurrences, and just 133 headwords (3.2%) recur. The one word spanning three volumes
is _abnormal_ (volumes 1, 17 and 21); the two-volume set reads like a who's who of general academic
verbs and nouns — _absorb, accident, accompany, acoustic, adapt, admission, adversity, anecdote,
assemble_ — i.e. the cross-volume residue is real general vocabulary, which is a reassuring sanity
check on the workbook's coverage strategy. The practical implications for learners and for any
vocabulary-learning system built on this corpus are that (a) books must be studied, not sampled —
reading one volume's list covers almost none of another's — and (b) recurrence is a genuine but
extremely weak difficulty signal, which is why the review planner's default ordering is the stable
dataset order rather than a recurrence ranking (§45).

### 44. The retention model: one curve, explicit parameters, no magic

The layer's retention calculator publishes the classic single-exponential forgetting curve
`R = exp(-days / stabilityDays)`, where `stabilityDays` is a free memory-strength parameter in
days. The curve is the standard first approximation to Ebbinghaus's savings data (Ebbinghaus 1885;
replicated with the original method by Murre and Dros 2015), whose anchors are: savings ≈ 0.58
after 20 minutes, 0.44 after 1 hour, 0.34 after 1 day, 0.25 after 6 days, 0.21 after 31 days. Two
caveats are published with the model rather than hidden: the classic values are _savings_
(relearning effort spared), not recognition or production scores, and a single exponential cannot
fit all seven anchor points at once (the data fall faster early and slower late than any one
curve), so the parameter is a tool for the researcher, not a constant of nature. The default
`stabilityDays = 1` is chosen only so that the out-of-the-box 24-hour prediction `e^(-1) ≈ 0.37`
sits near the one-day savings value of ≈ 0.34; the endpoint also reports the half-life
(`stabilityDays · ln 2`) and, given a `target` retention, inverts the curve to the last whole day
retention stays above it — the "review before this day" answer a scheduler needs.

The review planner extends the curve the way the spaced-practice literature does: after each
completed review the stability multiplies by a growth factor (default 2), so the `i`-th review of a
word (0-based) is predicted at `R = exp(-reviewDay / (stabilityDays · growth^i))`. This is the
expanding-rehearsal design of Landauer and Bjork (1978) — successful retrievals spaced at
increasing gaps — and the spacing benefit quantified by Cepeda et al. (2006). Every parameter of
the ladder (`reviewDays`, `stability`, `growth`) is exposed on the endpoint so a researcher can
plug in fitted values; the defaults are documented, not asserted.

### 45. The review calendar: scheduling 4,174 words without a user table

`/v1/study/review` answers "what should I learn and review each day?" as a pure function: no
accounts, no progress rows, no server state — which is what keeps the API free and
reproducible. A request fixes a UTC start date, a window of `days`, `newPerDay`, an optional
volume/part-of-speech scope and an ordering; day `d` receives a slice of the ordered scope of size
`min(newPerDay, scope)`, starting at position `(d · newPerDay) mod scope`. After
`ceil(scope / min(newPerDay, scope))` days the list has been covered once and the walk cycles, so
any window length is answerable and every calendar is byte-identical across replicas and years.
A word first studied on day `p` is scheduled for review on day `p + reviewDay` for each ladder
value; the window is anchored at the start date, so reviews are only listed for words first
studied inside the window — anything earlier would need the state the API deliberately does not
keep. Retention is attached to every due review under the model of §44, and the response states
in plain words that the model assumes every earlier review succeeded: a stateless API cannot know
whether a learner actually showed up. Scope filters follow the vocabulary endpoint's semantics (a
word matches when it appears in any selected volume); an empty or saturated scope yields a note
rather than an error, and `order` offers the dataset's stable list order, headword length, or the
recurrence ranking whose weak discriminative power §43 quantifies.

### 46. Practice items that are reproducible study aids, not assessments

`/v1/study/quiz` generates definition-recognition items entirely from the vocabulary dataset: the
learner sees a headword and must pick its definition from four options. The correct definition is
the entry's first sense text; the three distractors are definitions of _other_ headwords,
preferred from the same part of speech (a noun distracts from a noun) and falling back across
parts of speech when a filtered scope is too small; texts are deduplicated, and glosses are
truncated at 200 characters so that long WordNet-style definitions cannot leak the answer by
length. Item selection, distractor choice and option order are seeded by the date and the scope,
so the same request returns byte-identical sets and the correct-answer index is published with
each item: a client hides it until the learner answers. Three quality decisions deserve notice.
First, format honesty: these are self-test items over the corpus glosses, with no difficulty
calibration, no pilot data and no distractors chosen for semantic proximity — the distractor
pools are deterministic samples, so items measure whether someone can recognise the workbook
gloss, nothing more; that is why the endpoint's metadata and this part both say the set is a study
aid, not an assessment. Second, the reference system's junk examples (§42) are the counterexample:
this API generates nothing from templates, because templated text at scale is precisely what
produces "You can Abandon attempt.". Third, small scopes degrade gracefully: when fewer than four
definitions exist the item offers what exists and flags `reduced`, and an empty scope returns an
empty set with a note instead of an error.

### 47. Threats to validity (Part VIII)

- **The retention numbers are model outputs, not measurements.** The exponential curve with
  growth is a two-parameter working model anchored to 1885/2015 savings data; recognition and
  production memory decay differently, and real learners vary. Endpoints expose every parameter
  and the metadata repeats the caveat; RESEARCH claims for the model must fit their own data.
- **Recurrence is a weak signal.** 96.8% of headwords occur in exactly one volume, so the
  `recurrence` ordering is nearly a tie and any ranking built on it inherits the workbook's
  (unknown) within-volume ordering. The default ordering inherits the same unknown: dataset order
  is alphabetical, which is stable and reproducible but pedagogically arbitrary.
- **The calendar assumes perfect review.** No forgetting, no skipped sessions, no mastery levels —
  deliberately, because statelessness is the point. Schedules are plans under the model, and the
  retention attached to a far-future review assumes every earlier review happened.
- **Cycling duplicates words after one pass.** Long windows re-walk the scope; learners who want
  strict novelty must grow the scope (more volumes) or shrink `newPerDay`. The response reports
  `onePassDays` so the duplication point is visible, not hidden.
- **Items inherit the corpus glosses.** Definitions come from the workbook's explanation column
  (see Part I §5 for its provenance); they are not written as test items, may contain hints, and
  may be truncated at 200 characters. Distractors are sampled, not constructed for similarity, so
  some items are trivially easy and others misleadingly hard.
- **UTC days and date seeds.** A "day" is a UTC calendar day and the default `date` is today in
  UTC; a learner in another timezone may want to pass the date explicitly.
- **The reference study is a snapshot.** The vocabulary system under study is live and unlicensed;
  its content, database and behaviour may have changed since HEAD `1f5ad56` (2026-09-07), and the
  observations in §42 are claims about that snapshot, not about the system today.

### 48. Reproducing Part VIII

The layer has no dataset of its own to regenerate: every endpoint is a pure function of
`data/vocabulary.json`, whose derivation from the upstream workbook is pinned and re-run by CI
(Part I §6). Reproducing a result is therefore one HTTP request:

```bash
curl -s "http://localhost:3000/v1/study/retention?days=7&stability=7&target=0.5"
curl -s "http://localhost:3000/v1/study/review?date=2026-09-07&days=7&newPerDay=10&order=recurrence"
curl -s "http://localhost:3000/v1/study/quiz?date=2026-09-07&count=5&volume=18"
```

Identical requests return byte-identical responses on any replica, any Node version (ordering
comparisons are locale-independent by design) and any future release that keeps the dataset
byte-identical — which CI enforces. The same functions are exported from the package for
dependency use (`retentionResult`, `buildReviewSchedule`, `buildQuiz`). The reference-system study
of §42 was performed on a shallow clone of `Iamdacai/ielts-vocab-system` at commit `1f5ad56`; the
repository is unlicensed, so this part quotes only structural facts (table names, row counts,
field schemas) and none of its data, and the clone was used solely for observation.
