...
2. Add query parameters to `PARAMETERS` in `src/lib/openapi.ts` so the OpenAPI document stays
   ...
   parameters do not. The generator translates `:id` to OpenAPI `{id}` paths; update response
   schemas for typed endpoints and add a contract test that validates actual responses.
3. Add tests: the happy path, every filter, every default, and every error branch.
4. Run `npm run docs:generate`; snapshot tests reject stale `docs/openapi.json` or `docs/research.html`.
5. Update `README.md` (endpoint table, dataset table if the data changed).

## Adding data

Datasets live in `data/` as JSON and are generated by Python standard-library or TypeScript/Node scripts in
...
The practice inventory is generated by `npm run data:practice -- <tree.json> <output.json>`.
Use the exact commit/tree in `PRACTICE_SOURCE`; never infer licensing from public visibility. See
