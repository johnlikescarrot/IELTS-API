...
4. Run `npm run openapi:generate` and add contract tests for the new response schema. OpenAPI
   paths use `{parameter}` even though internal route templates use `:parameter`.
5. Update `README.md` (endpoint table, dataset table if the data changed).
...
- update the numbers in `README.md`, `RESEARCH.md` and, if the dataset changed shape,
  `paper/paper.md`;
- CI re-derives `data/vocabulary.json` from the upstream workbook and `data/practice.json` from
  the pinned practice Git tree; committed files must match their pipeline output exactly.
- The practice extractor is TypeScript (`npm run data:practice -- tree.json output.json`) and
  allowlists metadata paths only. Never fetch upstream environment/session files, student records,
  exercise payloads or media. Preserve missing-file and rights/access caveats.
- Run `npm run openapi:generate` after contract changes. Tests check the archived document and
  validate actual practice/error responses against the documented schemas.
