 ...
 * are fixtures rather than per-user storage. Supply now/endDate explicitly for
 * reproducibility; omitted dates use the current clock. No model is calibrated.
 */
...
const COMPARISON_META = {
  note: 'Illustrative, uncalibrated model output; not an IELTS or recall-probability prediction. Supply now/endDate explicitly for reproducibility. For versioned client-owned progress use /v1/study/review/policy.',
};
    ...
    data: {
      models: [
        ...
        },
        {
          ...
          provenance:
            'An illustrative variant inspired by Wozniak (1990); not Anki compatibility or the sm2-v1 state contract.',
          description:
            'Updates ease first, then rounds old interval × updated ease. Use /v1/study/review for the bounded, ceiling/old-ease policy.',
        },
      ],
      calendarLevels: {
        thresholdsMinutes: [0, 15, 30, 60],
        mapping: '0 = 0 min, 1 = 1-14, 2 = 15-29, 3 = 30-59, 4 = 60+.',
      },
      references: [
        'Ebbinghaus, H. (1885). Über das Gedächtnis.',
        'Leitner, S. (1972). So lernt man lernen.',
        'Wozniak, P. A. (1990). Optimization of learning.',
        'Cepeda, N. J. et al. (2006). Distributed practice in verbal recall tasks. Psychological Bulletin, 132(3), 354-380. doi:10.1037/0033-2909.132.3.354',
      ],
...
function schedule(context: RouteContext): HandlerResult {
  ...
  if (methodFilter !== undefined) {
    ...
    return { data: filtered as unknown as JsonValue, meta: COMPARISON_META };
  }

  return { data: data as unknown as JsonValue, meta: COMPARISON_META };
}
...
function calendar(context: RouteContext): HandlerResult {
  ...
  return {
    ...
    meta: { seed, days, endDate: effectiveEnd, levels: '0 = 0 min, 1 = 1-14, 2 = 15-29, 3 = 30-59, 4 = 60+' },
  };
}
...
function helpers(context: RouteContext): HandlerResult {
  ...
  return { data: result as unknown as JsonValue, meta: COMPARISON_META };
}
