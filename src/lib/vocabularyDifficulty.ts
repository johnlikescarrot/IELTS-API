 ...
 * features onto a 0-100 score, a C EFR-style band and a plain-language level
...
const LEVEL_THRESHOLDS: readonly { max: number; level: DifficultyLevel; label: string }[] = [
  ...
  { max: Number.POSITIVE_INFINITY, level: 'C2', label: 'Proficiency' },
];
...
function levelForScore(score: number): { level: DifficultyLevel; label: string } {
  ...
  // c8 ignore next -- thresholds cover all real scores; fallthrough for safety
