...
const PARAMETERS: Record<string, JsonValue[]> = {
  ...
  '/v1/wordbook/items': [
    QUERY,
    {
      name: 'book',
      in: 'query',
      description: 'Comma-separated Cambridge volumes (1-18) as attributed by the upstream wordbook.',
      schema: { type: 'string', example: '7,12' },
    },
    {
      name: 'shared',
      in: 'query',
      description: 'Keep only rows present in (true) or absent from (false) the Cambridge 1-22 list.',
      schema: { type: 'boolean' },
    },
    {
      name: 'agrees',
      in: 'query',
      description: 'Keep only rows whose claimed volume does (true) or does not (false) match the workbook.',
      schema: { type: 'boolean' },
    },
    { name: 'sort', in: 'query', schema: { type: 'string', enum: ['word', 'book'], default: 'word' } },
    { name: 'order', in: 'query', schema: { type: 'string', enum: ['asc', 'desc'], default: 'asc' } },
    LIMIT,
    OFFSET,
  ],
  '/v1/study/srs/next': [
    {
      name: 'ladder',
      in: 'query',
      description: 'Which published ladder to apply.',
      schema: { type: 'string', enum: ['classic', 'wheel'], default: 'classic' },
    },
    {
      name: 'review',
      in: 'query',
      description: 'Completed reviews of this word (ladder stage).',
      schema: { type: 'integer', minimum: 0, maximum: 64, default: 0 },
    },
    {
      name: 'mastery',
      in: 'query',
      description: 'Current mastery score; grows the interval past the last rung.',
      schema: { type: 'number', minimum: 0, maximum: 100, default: 0 },
    },
    {
      name: 'date',
      in: 'query',
      description: 'Anchor calendar date (YYYY-MM-DD, UTC); defaults to today.',
      schema: { type: 'string', format: 'date' },
    },
  ],
  '/v1/study/srs/grade': [
    {
      name: 'mastery',
      in: 'query',
      description: 'Mastery score before this review.',
      schema: { type: 'number', minimum: 0, maximum: 100, default: 0 },
    },
    {
      name: 'correct',
      in: 'query',
      required: true,
      description: 'Whether the recall attempt succeeded.',
      schema: { type: 'boolean' },
    },
    {
      name: 'confidence',
      in: 'query',
      description: 'Self-rated confidence, weighted into the mastery change.',
      schema: { type: 'integer', minimum: 1, maximum: 5, default: 3 },
    },
    {
      name: 'review',
      in: 'query',
      description: 'Completed reviews before this one.',
      schema: { type: 'integer', minimum: 0, maximum: 64, default: 0 },
    },
    {
      name: 'ladder',
  ...
  '/v1/study/srs/window': [
    {
      name: 'date',
      in: 'query',
      description: 'Calendar date of the study day; defaults to today.',
      schema: { type: 'string', format: 'date' },
    },
    {
      name: 'time',
      in: 'query',
      description: 'Review time of day, HH:MM.',
      schema: { type: 'string', pattern: '^([01]\\d|2[0-3]):([0-5]\\d)$', default: '20:00' },
    },
  ],
  '/v1/study/srs/project': [
    {
      name: 'words',
      in: 'query',
      description: 'Words in the book; defaults to the size of the indexed community wordbook.',
      schema: { type: 'integer', minimum: 1, maximum: 10000, default: 4323 },
    },
    {
      name: 'perDay',
      in: 'query',
      description: 'New words per study day (the upstream default is 20).',
      schema: { type: 'integer', minimum: 1, maximum: 100, default: 20 },
    },
    {
      name: 'ladder',
      in: 'query',
      description: 'Which published ladder to project.',
      schema: { type: 'string', enum: ['classic', 'wheel'], default: 'wheel' },
    },
    {
      name: 'start',
      in: 'query',
    ...
    },
    {
      name: 'horizon',
      in: 'query',
      description: 'Days of per-day detail to return; aggregates always cover the whole plan.',
      schema: { type: 'integer', minimum: 7, maximum: 180, default: 42 },
    },
  ],
  '/v1/study/plan': [
    {
  ...
  return {
    ...
    info: {
      ...
      description: [
        ...
        'raw-score-to-band calibration, and a community-wordbook index: 4,323',
        'headwords cross-validated against the Cambridge list, with a data-quality',
        'audit of the system they came from. The toolkit scores any text',
        '(readability and essay profile) and composes the datasets into study plans,',
        'including a stateless Ebbinghaus spaced-repetition scheduler.',
        '',
