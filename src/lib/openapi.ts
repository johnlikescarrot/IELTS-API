...
const PARAMETERS: Record<string, JsonValue[]> = {
  ...
  '/v1/practice/lessons': [
    QUERY,
    {
      name: 'series',
      in: 'query',
      description: 'Comma-separated series: listening-102, listening-204, reading-1232, reading-315.',
      schema: { type: 'string' },
    },
    {
      name: 'level',
      in: 'query',
      description: 'Comma-separated levels: Basic, Intermediate, Advanced, A1-A2, B1-B2, C1-C2.',
      schema: { type: 'string' },
    },
    {
      name: 'type',
      in: 'query',
      description: 'Comma-separated normalised item-type labels (see /v1/practice/types).',
      schema: { type: 'string' },
    },
    { name: 'skill', in: 'query', schema: { type: 'string', enum: ['listening', 'reading'] } },
    { name: 'kind', in: 'query', schema: { type: 'string', enum: ['lesson', 'full-test'] } },
    {
      name: 'sort',
      in: 'query',
      schema: {
        type: 'string',
        enum: ['id', 'series', 'level', 'number', 'questions', 'words'],
        default: 'id',
      },
    },
    { name: 'order', in: 'query', schema: { type: 'string', enum: ['asc', 'desc'], default: 'asc' } },
  ...
  return {
    ...
    info: {
      ...
      description: [
        ...
        'descriptors, score concordances, Writing and Speaking task banks, an index',
        'of the open IELTS research corpus, and a metadata index of the open IELTS',
        'practice corpus (1,804 items, CEFR-graded reading and listening material).',
        '',
