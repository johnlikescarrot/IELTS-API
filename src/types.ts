...
/** Result of the forgetting-curve calculator. */
export type RetentionResult = {
  /** The exponential model used. */
  formula: string;
  /** Days elapsed since learning. */
  days: number;
  /** Memory-stability parameter (days). */
  stabilityDays: number;
  /** Predicted retention `exp(-days / stabilityDays)`, rounded to 4 decimals. */
  retention: number;
  /** Half-life `stabilityDays * ln 2`, rounded to 2 decimals. */
  halfLifeDays: number;
  /** Inversion towards a target retention, when requested. */
  target: null | {
    /** Retention the researcher wants to keep. */
    retention: number;
    /** Exact days until retention decays to the target, rounded to 2 decimals. */
    daysUntil: number;
    /** Largest whole day with retention still at or above the target. */
    lastWholeDay: number;
    /** Retention predicted at {@link RetentionResult.target.lastWholeDay}. */
    retentionAtLastWholeDay: number;
  };
};

/** Ordering of the headword list used by the review planner. */
export type ReviewOrder = 'list' | 'length' | 'recurrence';

/** One headword scheduled on one day of a review plan. */
export type ScheduledWord = {
  /** Vocabulary identifier (`w00001`). */
  id: string;
  /** Headword. */
  word: string;
};

/** One due review within a review plan. */
export type DueReview = {
  /** Vocabulary identifier of the reviewed word. */
  id: string;
  /** Headword. */
  word: string;
  /** Days after first study at which this review falls. */
  reviewDay: number;
  /** Days since the word was first studied. */
  gapDays: number;
  /** Predicted retention under the schedule's stability model. */
  retention: number;
};

/** One day of a review plan. */
export type ReviewDay = {
  /** Calendar date (UTC, ISO-8601). */
  date: string;
  /** Zero-based index inside the window. */
  index: number;
  /** Words scheduled for first study on this day. */
  new: ScheduledWord[];
  /** Reviews due on this day for words first studied inside the window. */
  reviews: DueReview[];
  /** Per-day totals. */
  counts: { new: number; reviews: number };
};

/** A deterministic review plan over the vocabulary dataset. */
export type ReviewSchedule = {
  /** Window covered by the plan. */
  window: { from: string; days: number; newPerDay: number };
  /** Headwords scheduled (empty when the filter matches nothing). */
  scope: { headwords: number; onePassDays: number };
  /** Spacing and stability model applied. */
  schedule: {
    /** Days after first study at which each review falls. */
    reviewDays: readonly number[];
    /** Initial stability in days. */
    stabilityDays: number;
    /** Stability multiplier per completed review. */
    stabilityGrowth: number;
    /** Retention formula used for each due review. */
    formula: string;
  };
  /** Day-by-day plan. */
  days: ReviewDay[];
...
/** One generated multiple-choice vocabulary item. */
export type QuizItem = {
  /** Vocabulary identifier of the target headword. */
  id: string;
  /** Target headword. */
  word: string;
  /** Part of speech of the target headword. */
  pos: PartOfSpeech;
  /** Instruction shown with the item. */
  prompt: string;
  /** Definition options; exactly one matches the headword. */
  options: string[];
  /** Index of the matching option within {@link QuizItem.options}. */
  answer: number;
};

/** A generated practice set. */
export type Quiz = {
  /** Calendar date (UTC) that seeds the item selection. */
  date: string;
  /** Number of items requested. */
  count: number;
  /** Items actually generated (fewer when the scope is small). */
  items: QuizItem[];
  /** Scope the items were drawn from. */
  scope: { headwords: number };
  /** Number of options each full item offers. */
  optionsPerItem: number;
  /** Whether any distractor came from another part of speech. */
  crossPosFallback: boolean;
  /** Whether any item offers fewer than {@link Quiz.optionsPerItem} options. */
  reduced: boolean;
};
