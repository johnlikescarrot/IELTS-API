 ...
 *   The scene view is a deterministic organisation aid, not a validated
 *   semantic classification or evidence of improved retention.
 */
...
export function vocabularyCollectionsStats(): VocabularyCollectionsStats {
  ...
  const memberships = Object.values(counts).reduce((sum, count) => sum + count, 0);
  const mean = Math.round((memberships / VOCABULARY_COLLECTIONS.length) * 100) / 100;
